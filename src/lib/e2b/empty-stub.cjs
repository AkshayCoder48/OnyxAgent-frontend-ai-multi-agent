...
// the browser) by @e2b/code-interpreter. The SDK imports `tar`, `glob`, `fs`, `path`,
...
function throwingFn(name) {
  return function () {
    throw new Error(
      name +
        " is not available in the browser. This E2B SDK feature requires a Node runtime.",
    );
  };
}
...
module.exports = new Proxy(
  function () {},
  {
    get: function (_target, prop) {
      ...
      if (prop === "default") return throwingFn("e2b-stub:default");
      if (prop === "then") return undefined; // Avoid thenable detection.
      return throwingFn("e2b-stub:" + String(prop));
    },
    apply: function () {
      throw new Error(
        "e2b-stub is not available in the browser. This E2B SDK feature requires a Node runtime.",
      );
    },
  },
);
