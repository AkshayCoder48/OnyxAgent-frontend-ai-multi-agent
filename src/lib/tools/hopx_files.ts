...
/**
 * E2B sandbox file/workspace tools — 10 tools total.
 *
 * Mirrors the original Python `app/agents/tools/workspace_tools.py` but talks
 * straight to E2B from the browser (the user's per-account key is decrypted
 * at call-time by the services layer and passed via `ToolContext.hopxApiKey`
 * / `ToolContext.sandboxApiKey`).
 *
 * If no E2B key is configured, file ops transparently fall back to OPFS
 * (Origin Private File System) so the agent can still read/write files —
 * just locally to this browser. `send_file` and `send_folder` return a
 * `file_download` payload with a `download_url` that the frontend's
 * `FileDownloadResult` card can fetch.
 *
 * Tools (10):
 *   - list_files
 *   - read_file
...
function assertE2BKey(ctx: ToolContext): string | null {
  // `hopxApiKey` is the legacy field name; `sandboxApiKey` is the new alias.
  // Both hold the same E2B sandbox API key.
  return ctx.hopxApiKey ?? ctx.sandboxApiKey ?? null;
}
...
registerTool(
  ...
  async (args, ctx) => {
    ...
    if (key) {
      const client = getHopxClient(key, ctx.conversationId, ctx.sandboxMode ?? "shared");
...
registerTool(
  ...
  async (args, ctx) => {
    ...
    if (key) {
      ...
      if (!overwrite) {
        try {
          ...
          if (existing !== undefined && existing !== "") {
            throw new Error(`File already exists: ${path} (use overwrite=true)`);
          }
        } catch (err) {
          // readFile throws on missing file — that's what we want.
      ...
      // Split path into directory + filename for OPFS.
      const parts = path.split("/");
      const filename = parts.pop()!;
      const subdir = parts.join("/");
      if (!overwrite) {
        try {
          await opfs.readFile(opfsPath);
          throw new Error(`File already exists: ${path} (use overwrite=true)`);
        } catch (err) {
      ...
      await opfs.writeFile(ctx.userId, `workspace/${subdir ? `${subdir}/` : ""}`, filename, content);
    }
...
registerTool(
  ...
  async (args, ctx) => {
    ...
    if (key) {
      ...
      const filename = parts.pop()!;
      const subdir = parts.join("/");
      await opfs.writeFile(ctx.userId, `workspace/${subdir ? `${subdir}/` : ""}`, filename, updated);
      return { path, replacements: count };
    }
  },
  false,
