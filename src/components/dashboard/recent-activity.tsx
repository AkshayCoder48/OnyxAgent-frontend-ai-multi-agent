...
export function RecentActivity({ limit = 6 }: { limit?: number }) {
  ...
  const load = async () => {
    ...
    setItems(null);
    try {
      // Backendless: there is no billing/credits ledger. Only the user's
      // recent conversations are surfaced here. If the user isn't signed in
      // yet (auth still loading), show an empty state instead of erroring.
