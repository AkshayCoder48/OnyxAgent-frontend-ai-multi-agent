...
const E2B_KEY_RE = /^[A-Za-z0-9_\-]{8,}$/;

export function SectionE2B() {
  const { settings, loading, setE2BKey } = useSettings();
  const [key, setKey] = React.useState("");
  const [show, setShow] = React.useState(false);
  const [saving, setSaving] = React.useState(false);
  const [clearOpen, setClearOpen] = React.useState(false);

  const hasStored = !!settings?.sandbox_api_key_present || !!settings?.e2b_api_key_present;
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="e2b-key">E2B Sandbox API key</Label>
